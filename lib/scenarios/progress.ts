...
export function calculateProgressPercentages(
  ...
  return Object.fromEntries(scenario.progress.map((definition) => {
    const current = Math.min(definition.max, Math.max(definition.min, metrics[definition.key] ?? definition.start));
    const delta = definition.direction === 'higher-is-better'
      ? current - definition.start
      : definition.start - current;
    return [definition.key, Math.min(100, Math.max(0, (delta / Math.max(1, Math.abs(definition.target - definition.start))) * 100))];
  }));
}
