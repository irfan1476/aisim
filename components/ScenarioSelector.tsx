...
export default function ScenarioSelector({
  ...
  const currencyButtons = (
    <div className="flex flex-wrap gap-2" role="group" aria-label="Display currency">
  ...
  return (
    ...
    <div className="flex min-w-0 flex-wrap items-start justify-between gap-4">
      ...
      <div className="mt-5 grid gap-3 sm:grid-cols-2" role="group" aria-label="Operating mode">
        ...
        <button type="button" aria-pressed={enabled} onClick={() => onEnabledChange(true)} className={`rounded-xl border p-4 text-left text-ink transition focus:outline-none focus:ring-2 focus:ring-emerald/40 ${enabled ? 'border-emerald bg-emerald/10 shadow-md ring-2 ring-emerald/20' : 'border-emerald/35 bg-emerald/[.03] hover:border-emerald'}`}>
