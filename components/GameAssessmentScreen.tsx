...
export default function GameAssessmentScreen({
  assessment,
  onAssessmentChange,
  onComplete,
  canContinue,
      ...
      <section className="mx-auto max-w-3xl pt-12">
        ...
        <div className="mt-8 space-y-4">
          {questions.map((question, index) => (
            <div
              key={question}
              data-testid={`baseline-question-${index}`}
            ...
            >
              ...
              <div className="mt-4 grid grid-cols-5 gap-2">
                {[1, 2, 3, 4, 5].map((value) => (
                  <button
                    key={value}
                    data-testid={`baseline-${index}-${value}`}
                    onClick={() => onAssessmentChange(index, value)}
                    className={`rounded-lg border py-2 text-sm font-bold ${assessment[index] === value ? "border-gold bg-gold text-ink" : "border-ink/10 bg-mist text-ink/60"}`}
                  >
                    {value}
                  </button>
                ))}
        ...
        <button
          ...
          onClick={onComplete}
          className="mt-7 flex w-full items-center justify-center gap-2 rounded-xl bg-ink px-5 py-4 text-sm font-bold text-white disabled:opacity-35"
        >
          Enter the boardroom <ArrowRight size={16} />
        </button>
      </section>
      {analytics}
    </main>
