...
export default function LearningRetrospective({ state }: { state: any }) {
  ...
  const recordedAllocation = latest.allocation || latest.alloc || state.alloc || {};
  const operatingMix = Object.entries(recordedAllocation).sort(([, left], [, right]) => Number(right) - Number(left)).slice(0, 3).map(([key, value]) => `${key === 'mlops' ? 'Ops & maintenance' : key[0].toUpperCase() + key.slice(1)} ${value}%`).join(' · ');
  ...
  return <section className="rounded-xl border border-emerald/30 bg-emerald/10 p-5">
    ...
    <div className="mt-4 grid gap-3 sm:grid-cols-3">
      ...
      <div className="rounded-lg bg-ink/30 p-3"><p className="text-[10px] uppercase tracking-wider text-white/40">Observed change</p><p className="mt-2 text-sm text-white/75">{metric ? `${metric.definition.label}: ${metric.progress.toFixed(0)}% toward target` : `Portfolio ROI: ${Number(latest.metrics?.roi || 0).toFixed(1)}%`}</p><p className="mt-2 text-xs text-white/45">Operating mix recorded: <span className="text-white/70">{operatingMix || 'No allocation recorded'}</span></p><p className="mt-2 text-xs text-white/45">Use History for the full metric and allocation ledger.</p></div>
