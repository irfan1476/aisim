...
export default function LearningRetrospective({ state }: { state: any }) {
  ...
  const recommendations = latest.recommendations || latest.proactiveRecommendations || [];
  ...
  return <section className="rounded-xl border border-emerald/30 bg-emerald/10 p-5">
    ...
    <div className="mt-4 grid gap-3 sm:grid-cols-3">
      ...
      <div className="rounded-lg bg-ink/30 p-3"><p className="text-[10px] uppercase tracking-wider text-white/40">Next question</p><p className="mt-2 text-sm text-white/75">{state.nextQuarterGuidance?.action || (metric ? `What would move ${metric.definition.label.toLowerCase()} next?` : 'Which capability needs the next investment?')}</p><p className="mt-2 text-xs text-emerald">{approved.length ? `${approved.length} recommendation${approved.length === 1 ? '' : 's'} approved` : 'No recommendations approved yet'}</p></div>
    </div>
