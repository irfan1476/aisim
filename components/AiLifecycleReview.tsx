              ...
              {statuses.map((kind) => {
                ...
                if (kind === "evaluation") {
                  ...
                  return <form key={kind} className="mt-4 rounded-xl border border-[#f0a736]/30 bg-[#f0a736]/[.06] p-3" onSubmit={(event) => { event.preventDefault(); onEvaluation({ initiativeId: item.id, decision: (draft as any).decision || "go", rationale: draft.rationale.trim(), owner: draft.owner.trim() }); }}>
                    ...
                    {signals.evaluation?.recommendedDecision === "go_with_conditions" && <p className="mt-1 text-[10px] leading-4 text-[#b6e3c0]">A bounded rollout is viable: select <b>Go</b> to continue, then choose augmentation or automation. A No-Go or Pause is still available if you want more evidence.</p>}
