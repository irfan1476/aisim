        ...
        {selected.map((item) => {
          ...
          return (
            <article key={item.id} className="rounded-2xl border border-[#30363d] bg-[#161b22] p-4">
              ...
              <div className="mt-3 grid grid-cols-2 gap-2 border-t border-white/10 pt-3 text-[10px] sm:grid-cols-5">
              ...
              {statuses.map((kind) => {
                ...
                if (kind === "deployment") return <form key={kind} className="mt-4 rounded-xl border border-[#7ee787]/30 bg-[#1a7f37]/[.06] p-3" onSubmit={(event) => { event.preventDefault(); if (!draft.rationale.trim()) return; onDeployment({ initiativeId: item.id, mode: draft.mode, rationale: draft.rationale.trim() }); }}><p className="text-xs font-bold">Deployment mode</p><div className="mt-3 grid grid-cols-2 gap-2">{(["augmentation", "automation"] as const).map((mode) => <button key={mode} type="button" onClick={() => updateDraft(item.id, { mode })} className={`rounded-lg border p-2 text-left text-[10px] ${draft.mode === mode ? "border-[#7ee787] bg-[#1a7f37]/25 text-[#7ee787]" : "border-white/15 text-white/55"}`}><b className="block uppercase">{mode}</b><span className="mt-1 block leading-4">{mode === "augmentation" ? "AI assists human judgement." : "AI executes within defined boundaries."}</span></button>)}</div><label className="mt-3 block text-[10px] font-bold uppercase tracking-wide text-white/50">Rationale<textarea required minLength={8} value={draft.rationale} onChange={(event) => updateDraft(item.id, { rationale: event.target.value })} className="mt-1 min-h-14 w-full rounded-lg border border-white/15 bg-[#0d1117] p-2 text-xs font-normal normal-case tracking-normal text-white outline-none focus:border-[#7ee787]" placeholder="Why is this mode appropriate?" /></label><button type="submit" className="mt-3 w-full rounded-lg bg-[#2da44e] px-3 py-2 text-xs font-bold text-white">Record deployment mode</button></form>;
