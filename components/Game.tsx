...
import { createInitiativeGeneration, type ScenarioArchetype } from '../lib/game/generator';
...
export default function Game() {
  ...
  const [archetype, setArchetype] = useState<ScenarioArchetype>('balanced');
  const total = useMemo(() => Object.values(s.alloc).reduce((a, b) => a + b, 0), [s.alloc]);
  const availableInitiatives = useMemo(() => Object.values(s.initiativeStates || {}), [s.initiativeStates]);
  const liveInitiatives = useMemo(() => availableInitiatives.map((initiative) => ({ ...initiative, roi: compactNumber(initiative.currentRoi ?? initiative.roi), data: compactNumber(initiative.currentData ?? initiative.data), cost: compactNumber(initiative.currentCost ?? initiative.cost), risk: initiative.currentRisk ?? initiative.risk })), [availableInitiatives]);
  ...
  const ask = async () => { if (!question.trim() || isAsking) return; const userQuestion = question; setQuestion(''); setAnswer(''); setIsAsking(true); const replies: Record<string, string> = { CFO: `At ${s.roi.toFixed(1)}% ROI and $${s.spent.toFixed(1)}M spent, your next question is payback. Keep people above 15% to protect value realization.`, CTO: `Data readiness is ${s.data}%. In this ${s.initiativeGeneration?.archetype || 'balanced'} scenario, dynamic initiative risk is shaped by your data maturity.`, CHRO: `Adoption is ${s.adoption}%. Your people allocation is ${s.alloc.people}%; invest in enablement if you want the transformation to stick.`, RISK: `Risk exposure is ${s.risk}%, while compliance is ${s.compliance}%. In this context, initiative risk is adjusted for organizational, data, and team maturity.` }; try { const response = await fetch('/api/llm/chat', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ ...llm, messages: [{ role: 'system', content: `You are the ${persona} board advisor in an executive AI investment simulation. Reply in plain text only, under 120 words, using exactly these labels: Recommendation:, Why:, Next step:. Do not use Markdown, headings, or repeat the question. Be concise and actionable. Reference the selected scenario archetype, maturity context, dynamic initiative stats, and synergy pairs when relevant. Current state: ${JSON.stringify({ ...s, advisorContext: { generation: s.initiativeGeneration, dynamicInitiatives: availableInitiatives.map(item => ({ id: item.id, roi: item.currentRoi, cost: item.currentCost, risk: item.currentRisk, riskScore: item.riskScore, data: item.currentData, human: item.currentHuman, synergies: item.synergies })), recentHistory: s.history?.slice(-3) || [], selected: s.selected || [], allocation: s.alloc || {}, causalChain: s.causalChain || [], recommendations: s.proactiveRecommendations || [] } })}` }, { role: 'user', content: userQuestion }] }) }); const data = await response.json(); if (!response.ok) throw new Error(data.error); setAnswer(data.content || replies[persona]); setIsAsking(false); } catch { setAnswer(replies[persona]); setIsAsking(false); } };
  ...
  if (screen === 'setup') return <GameSetupScreen name={name} experimental={experimental} archetype={archetype} onNameChange={setName} onExperimentalChange={setExperimental} onArchetypeChange={setArchetype} onContinue={() => setScreen('assessment')} />;
  if (screen === 'assessment') return <GameAssessmentScreen assessment={assessment} onAssessmentChange={(index, value) => setAssessment(current => { const next = [...current]; next[index] = value; return next; })} onComplete={() => { const generation = createInitiativeGeneration(archetype, assessment); store.resetCampaign(); store.loadGame({ ...initialGameState(generation), baseline: assessment, experimental }); setScreen('game'); }} canContinue={assessment.length === 5} analytics={<AnalyticsHub state={s} initiatives={availableInitiatives} />} />;
