...
export default function StrategyDNA({ state }: { state: any }) {
  ...
  return <section className="space-y-4 rounded-2xl border border-fuchsia-400/30 bg-fuchsia-400/10 p-5">
    <div className="flex items-center gap-2"><GitBranch size={17} className="text-fuchsia-200"/><div><h3 className="font-bold text-fuchsia-200">Strategy DNA</h3><p className="mt-1 text-xs text-white/55">A compact interpretation of the strategy you actually practiced. It is evidence-led, not a fixed player type.</p></div></div>
    ...
    <div className="grid gap-2 text-xs text-white/60 sm:grid-cols-3"><p>Evidence: {history.length} completed quarters</p><p>Average capability allocation: {people.toFixed(1)}% people · {data.toFixed(1)}% data</p><p>Average governance allocation: {compliance.toFixed(1)}%</p></div>
    ...
    <p className="rounded-lg bg-black/15 p-3 text-xs text-white/70">Current tension: <b className="text-fuchsia-200">{weakest?.label || 'portfolio balance'}</b> is the least advanced outcome against its target. Your latest pattern is <b className="text-fuchsia-200">{latest?.posture || 'not yet established'}</b>. Compare this with Evolution to see which initiatives changed, rather than treating this interpretation as a diagnosis.</p>
    <p className="text-[10px] text-white/40">Modelled interpretation from recorded allocations, portfolio breadth, outcomes, and risk. Initiative-level changes belong in the Evolution tab.</p>
  </section>;
}
