...
export default function LLMSettings() {
  ...
  return <div className="rounded-2xl border border-ink/8 bg-white p-4">
    ...
    <div className="mt-4 grid gap-3 sm:grid-cols-2"><label className="text-xs font-bold text-ink/55">Model{settings.provider === 'llama' ? <input type="text" value={settings.model} onChange={(e) => settings.set({ model: e.target.value })} placeholder="Enter model name (e.g., gemma4:e4b)" className="mt-1 w-full rounded-lg border border-ink/10 bg-mist px-3 py-2 text-sm" /> : <select value={settings.model} onChange={(e) => settings.set({ model: e.target.value })} className="mt-1 w-full rounded-lg border border-ink/10 bg-mist px-3 py-2 text-sm">{current.models.map((model) => <option key={model}>{model}</option>)}</select>}</label><label className="text-xs font-bold text-ink/55">Temperature <span className="font-normal">{settings.temperature.toFixed(1)}</span><input type="range" min="0" max="1" step=".1" value={settings.temperature} onChange={(e) => settings.set({ temperature: Number(e.target.value) })} className="mt-3 w-full accent-[#D4AF37]" /></label></div>
    {settings.provider === 'llama' && <div className="mt-2 rounded-lg border border-emerald/20 bg-emerald/8 px-3 py-2 text-[11px] leading-5 text-ink/60">💡 Ollama tip: Enter your model name exactly as it appears in <code>ollama list</code>. Example: <code>gemma4:e4b</code> or <code>llama3.2:3b</code></div>}
