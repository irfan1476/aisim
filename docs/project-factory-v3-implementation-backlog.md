...
| UX-W1-08 | In progress — Predictive Maintenance reference drafted | Add a decision-context object and packet presentation for each opening priority: why now, known facts, unknowns, decision test, accountable role, if-deferred trigger, cost/capacity, and plain-language terms. The Predictive Maintenance packet is now the reference; complete equivalent visible context for Visual Quality and Technician Knowledge, then validate with a non-manufacturing reviewer. |
