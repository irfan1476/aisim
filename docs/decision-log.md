...
## 2026-08-22 — Window 1 dedicated V3 learner shell implemented

Status: implemented local vertical slice; broader orchestration and pilot remain open

- **Product-owner direction:** After the local V2-like cockpit was judged confusing and disconnected, implement the approved dedicated V3 Window 1 journey rather than adding more panels to the legacy screen. The learner-facing method is `Orient → Compare → Commit → Outcome → Reflect → Next Window` with one primary action per state.
- **Implemented:** `components/V3WindowShell.tsx` is now the opt-in V3 active-play shell. It presents the board question, three bounded Research priorities, co-located evidence/cost/capacity/timing/owner/boundary information, structured prediction, resolver-authored Research outcome, optional reflection, and a clear carry-forward into Window 2. The legacy V2 cockpit, generic advisor, full Evidence Room, and active-play sidecar are not rendered on this V3 path.
- **Runtime/content seam:** `lib/scenarios/projectFactoryV3.ts` now exposes authored Window 1 metadata; `components/Game.tsx` routes an opted-in V3 pack to the dedicated shell; `stores/gameStore.ts` provides an additive `advanceV3Window` hook. Evidence citation is captured in the existing V3 decision ledger through `confirmV3Decisions`.
- **Compatibility:** Standard mode and non-V3 scenarios continue through their existing path. No V2 resolver or generic selector was changed to make the V3 shell work.
- **Verification:** Type-check and production build pass; the existing unit/regression suite passes 68/68 with the generated report restored. A manual browser walkthrough on `http://127.0.0.1:5174` completed Orient → Compare → Commit → Outcome and returned the app to a clean V3 setup screen. The dedicated Playwright command could not execute because the environment's escalated browser-run permission review timed out; this is recorded as pending verification, not as a passing E2E result.
...
## 2026-08-22 — V3 implementation checkpoint paused for product-owner return

Status: implementation in progress; local changes uncommitted and not pushed

- **User direction:** Pause the implementation pass while the product owner steps away. Record the exact checkpoint and resume later; do not claim completion or push yet.
- **Completed in this pass:** Added authored Research-result branch metadata for all three Window 1 priorities; made Research operationally non-mutating; attached resolver-authored outcomes to the immutable decision ledger; added Q1–Q3 window snapshots/history and a persisted V3 cursor; persisted V3 baseline responses and Window 1 reflection; added capacity-by-quarter schedule bookkeeping; added event provenance fields; added additive board-memo and responsible-impact state actions; scoped Project Factory causal rules to their initiatives; added validator, persistence, runtime, resolver, and Window 1 contract coverage.
- **UX continuity:** The dedicated V3 shell now reads the saved phase cursor and writes phase changes, so a reload can resume Orient/Compare/Commit/Outcome/Reflect/Next rather than silently returning to the beginning. The Window 2 preview boundary remains explicit because later-window content is not authored.
- **Compatibility constraint:** All changes are opt-in to `v3State`; Standard mode, V1/V2 paths, and legacy resolver behavior remain unchanged. The active-play analytics sidecar remains excluded from the learner decision path.
- **Current worktree:** Branch `codex/project-factory-v3-impl`, based on frozen V2 commit `dc34433`, is 28 commits ahead of origin. The current implementation pass has uncommitted changes in V3 state/runtime/content/store/UI/tests plus the new `V3BaselineReflectionScreen` and `v3-window-contract` test. `balance-report.json` is a generated report change and should be restored before the implementation commit unless intentionally regenerated.
- **Next execution steps:** (1) run `npm test`, `npm run type-check` with incremental writes disabled if needed, `npm run build`, and the Playwright/browser flow; (2) manually inspect the local V3 Window 1 loop and save/resume cursor; (3) restore generated report noise; (4) commit with a clear V3 implementation message; (5) push `codex/project-factory-v3-impl` only after verification.
- **Checkpoint verification:** `git diff --check` and `npx tsc --noEmit --incremental false` pass after the latest changes. Full unit, build, and browser verification remain to be run after resuming.
- **Explicitly deferred:** Window 2–4 authored gameplay and pilot planning; quality/OEM and technician-retirement event cards pending operations review; P3 reuse/conversion for BankNext, Care360, FutureReady, or BharatMart; AI reflection coach; composite scoring; and any V2 integration or modification.
- **Decision needed on resume:** No product decision is currently required to finish the listed verification and commit/push steps. A decision gate resumes only if implementation encounters a regression in Standard/V2 behavior, missing/ambiguous authored content, or a request to cross into the deferred Window 2–4/P3 scope.

## 2026-08-28 — Window 1 V3 implementation and verification completed

Status: implemented on `codex/project-factory-v3-impl`; ready for local commit/push

- **Implementation outcome:** The additive V3 reference slice now converts the provisional Project Factory pack into runtime metadata, resolves the authored Window 1 Research contract across Q1–Q3, records Research-result branches, keeps Research from changing operating metrics, persists baseline/reflection/cursor/window history, records quarter-specific capacity use, attaches resolver-authored ledger outcomes, and preserves event provenance.
- **Learner flow verified:** The dedicated shell exercises `Orient → Compare → Commit → Outcome → Reflect → Next Window`; the browser walkthrough confirms the evidence packet, one Research decision, Pilot-ready finding, unchanged operating metrics, persisted reflection, saved cursor after reload, and the explicit non-authored Window 2 boundary. The active-play analytics sidecar remains hidden.
- **Compatibility verified:** Standard mode and legacy V1/V2 scenario paths pass without using the V3 shell or V3 resolver. No V2 code was modified.
- **Content correction:** Project Factory causal rules are initiative-scoped so Quality and Workforce effects cannot apply during the wrong lifecycle. The two held event cards remain deferred; no new event content was invented.
