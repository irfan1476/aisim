...
Status: agreed
Project Factory is the first v3 reference conversion. BankNext, Care360, and FutureReady follow; BharatMart is the first new-domain pack after the four existing scenarios are deepened. The rationale and Project Factory-specific design are in [Project Factory 2030 v3: Reference-Pack Design Brief](./project-factory-v3-design-brief.md).

Related records:

- [Scenario Learning Research and Architecture](./scenario-learning-research-and-architecture.md)
- [Scenario Pipeline — Implementation Plan](./scenario-pipeline-v3-reference.md)
...
## Audit findings that drive the v3 proposal

These are design boundaries, not implementation defects.

| Current behaviour | Learning limitation | v3 response |
|---|---|---|
...
## Architecture decision: additive v3, not a rewrite

The engine remains scenario-agnostic and data-driven. V3 fields are additive and optional so the existing packs retain their current behaviour until each is deliberately converted.
...
Existing ScenarioDefinition / ScenarioState
             ...
             │
             ▼
ScenarioPack v3 (optional depth data)
...
| `ScenarioState` | decision ledger, initiative lifecycle, reserved/actual budget, capacity, stakeholder state, governance evidence/gates, delayed effects, event log, score components | Persist under a versioned v3 state object; migrate v5 saves to safe default values. |
...
All four scenarios receive the same v3 structural language. They should not receive identical rules, risk weights, or learning objectives.
...
1. **Project Factory** becomes the reference v3 pack. It has concrete operations, measurable lagged outcomes, manageable safety risk, and clear shared-data dependencies.
...
**Exit condition:** approved v3 schema and a reviewed Project Factory scenario brief.

### Phase 1 — compatibility foundation

- Introduce optional v3 schema fields and a pack validator.
...
**Exit condition:** all existing scenarios, saves, Standard mode, and current tests behave identically when v3 content is absent.
...
**Exit condition:** pure module tests demonstrate reproducible, explainable effects across a representative v3 fixture.
